...
//#elif _delivery
#else
        Delivery.DeliveryForm mainform;
#endif
        public MapCtrl()
        {
            InitializeComponent();
        ...
        bool m_bDeliverymode = true;

        string strSelectedMissionID = "";
#if _fleet || _fleetdemo
        public MapCtrl(FleetManager_MainForm frm)
           {
               mainform = frm;
               InitializeComponent();
           }

          public void onSet_SelectedMissionID(string missionid)
        {
            strSelectedMissionID = missionid;
        }

#else
        public MapCtrl(Delivery.DeliveryForm frm)
        {
            mainform = frm;
            InitializeComponent();
        }
...
#if _delivery 
            txtXcell.Visible = false;
            txtYcell.Visible = false;

            txtXpos.Visible = false;
            txtYpos.Visible = false;

            chkAngluar.Visible = false;
            chkInitPosSet.Visible = false;
            txtAngluar.Visible = false;

#endif 
        }

        bool m_bMapDp = false;
        bool m_bCostMapDp = false;
        bool m_bRobotPathDp = false;
        string m_strMapID = "";


        public void onInitSet(bool mapdp,bool costmapdp, bool bpathdp,string mapid)
        {
        ...
        private void btnMaploading_Click(object sender, EventArgs e)
        {
            if (Data.Instance.isConnected)
            {
                ...
                if (cboRobotID.SelectedIndex < 0) return;
        ...
        private void pb_map_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (chkDelivery.Checked)
            {
                if (Data.Instance.isConnected)
                {
                    onDeliveryRun(currXpos, currYpos * -1, (float)0.7);
                }
            }
        ...
        private async void onDeliveryRun(float s_x, float s_y, float robotspeedvalue)
        {
            ...
            string[] strSelectworkdata_Worker = new string[1];

            strSelectworkdata_Worker[0] = string.Format("type:Goal-Point/x:{0}/y:{1}/theta:0/qual:C/max_trans_vel:{2:f1}" +
                        "/xy_goal_tolerance:0.18/yaw_goal_tolerance:0.05/p_drive:0.75/d_drive:1.2/wp_tolerance:1/avoid:true", s_x, s_y, robotspeedvalue);
        ...
        private async void onDelvery_Stop()
        {
            string strRobot = cboRobotID.SelectedItem.ToString();
            ...
            else
            {
                ...
                if (bPosRectSelect)
                {
                    if (posRectList.Count > 0)
                    {
                        ...
                        //mainform.onActionDataDP(nPosRectidx);
...
#region  cost map filter
        ...
        private void btnCostmap_Click_1(object sender, EventArgs e)
        {
