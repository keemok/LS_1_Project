...
namespace SysSolution
{
	...
	public class TreeViewSerializer
	{
        ...
        public void SerializeTreeNode(TreeNode treenode, string fileName)
        {
            XmlTextWriter textWriter = new XmlTextWriter(fileName, System.Text.Encoding.ASCII);
            // writing the xml declaration tag
            textWriter.WriteStartDocument();
        ...
        public TreeNode LoadXmlFileInTreeNode_sub(string fileName)
        {
            XmlTextReader reader = null;
            TreeNode n = new TreeNode("WORK");
            try
            {
                //treeView.BeginUpdate();
               ...
               // treeView.Nodes.Add(n);
                while (reader.Read())
                {
                    if (reader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            if (text.ToString().Equals("Work_List"))
                            { }
                            else n = n.Nodes.Add(text.ToString());
                        }
                    }
                    else if (reader.NodeType == XmlNodeType.EndElement)
                    {
        ...
        public void LoadXmlFileInTreeView(TreeView treeView, string fileName)
        {
            ...
            try
            {
                ...
                //TreeNode n = new TreeNode(fileName);
                TreeNode n = new TreeNode("WORK_LIST");
                treeView.Nodes.Add(n);
                while(reader.Read())
                {
                    if(reader.NodeType == XmlNodeType.Element)
